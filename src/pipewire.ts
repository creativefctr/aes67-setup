...
import { Aes67DeviceConfig, PipewireRoutingState } from "./types.js";

interface PwDumpEntry {
  id: number;
  type: string;
  info?: {
...
interface PipewireGraph {
  nodes: Map<number, PipewireNode>;
  ports: Map<number, PipewirePort>;
}
...
const sanitizeNodeName = (value: string): string => {
  return value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-+|-+$)/g, "")
...
const fetchGraph = async (logger: Logger): Promise<PipewireGraph> => {
  ...
  let parsed: PwDumpEntry[];
  try {
    parsed = JSON.parse(stdout) as PwDumpEntry[];
  } catch (error) {
    logger.error(`Failed to parse pw-dump output: ${(error as Error).message}`);
    throw new Error("Unable to parse PipeWire graph (pw-dump). Ensure pw-dump is available and returns JSON.");
  }

  const nodes = new Map<number, PipewireNode>();
  const ports = new Map<number, PipewirePort>();
...
const findSinkNode = (graph: PipewireGraph, logger: Logger, soundCardName: string): PipewireNode | undefined => {
...
const findSessionNode = (
  ...
  logger.warn(
    `Unable to locate PipeWire session node for '${sessionName}'. Dumping available node names: ${Array.from(graph.nodes.values())
...
const orderPorts = (ports: PipewirePort[], direction: "input" | "output"): PipewirePort[] => {
  return ports
...
const matchPorts = (
  sessionPorts: PipewirePort[],
  sinkPorts: PipewirePort[],
  channelNames: string[],
  logger: Logger,
): Array<{ output: PipewirePort; input: PipewirePort }> => {
  ...
  if (sinkPorts.length < sessionPorts.length) {
    logger.warn(
      `Sink only has ${sinkPorts.length} inputs while session provides ${sessionPorts.length} outputs. Excess outputs will be dropped.`,
    );
  }

  const normalizedChannels = channelNames.map((name) => normalize(name));

  const matches: Array<{ output: PipewirePort; input: PipewirePort }> = [];
  const usedInputs = new Set<number>();

  const findInputForChannel = (channel: string, fallbackIndex: number): PipewirePort | undefined => {
  ...
  sessionPorts.forEach((sessionPort, index) => {
    ...
    if (!input) {
      logger.warn(
        `No available sink input port found for session output ${sessionPort.props["port.name"] ?? sessionPort.id}. Skipping link.`,
      );
      return;
    }
...
const loadRtpSessionModule = async (
...
const waitForSessionNode = async (
  logger: Logger,
  sanitizedName: string,
  sessionName: string,
  retries = 10,
...
const createLink = async (outputPortId: number, inputPortId: number): Promise<number> => {
...
export const setupPipewireRouting = async (
  config: Aes67DeviceConfig,
  logger: Logger,
  ...
  logger.info(
    `Linked ${linkIds.length} PipeWire ports from RTP session '${config.sessionName}' to sink '${sinkNode.props["node.description"] ?? sinkNode.props["node.name"] ?? sinkNode.id}'.`,
  );
...
export const teardownPipewireRouting = async (state: PipewireRoutingState | undefined, logger: Logger): Promise<void> => {
  if (!state) {
    return;
  }
  ...
  for (const linkId of state.linkIds) {
    try {
      ...
      logger.warn(`Failed to remove PipeWire link ${linkId}: ${(error as Error).message}`);
    }
  }
  ...
  if (state.moduleId) {
    try {
      ...
      logger.warn(`Failed to unload PipeWire module ${state.moduleId}: ${(error as Error).message}`);
    }
  }
};
