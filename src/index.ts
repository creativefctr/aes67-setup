...
const getWindowsNetworkInterfaces = async (): Promise<Array<{ name: string; description: string; status: string }>> => {
  try {
    const { stdout } = await execCommandSafe("wmic path win32_networkadapter get name,netconnectionstatus /format:csv");
    const lines = stdout.split("\n").filter(line => line.trim() && !line.startsWith("Node"));
    
    const interfaces: Array<{ name: string; description: string; status: string }> = [];
        ...
        if (name && name !== "Name" && name.length > 0) {
          ...
          if (!isVirtualAdapter) {
            ...
            switch (statusCode) {
              ...
              case "7": status = "Disconnected"; break;
              case "0": status = "Disconnected"; break;
              case "": status = "Unknown"; break;
              default: status = `Status ${statusCode}`; break;
            }
...
const handleSenderSetup = async (configPath: string, logger: Logger): Promise<Aes67DeviceConfig> => {
  ...
  let jackClientName: string | undefined;
  let asioDeviceClsid: string | undefined;
  ...
  if (senderAnswers.audioSource === "jack") {
    ...
    const asioAnswers = await inquirer.prompt<{
    ...
    }>([
      {
        ...
        validate: (input: string) => {
          if (input.trim().length === 0) return "ASIO device CLSID cannot be empty";
