...
const parseAplayOutput = (output: string): SoundCardInfo[] => {
  const cards: SoundCardInfo[] = [];
...
export const listSoundCards = async (): Promise<SoundCardInfo[]> => {
  const { stdout } = await execCommandSafe("aplay -l");
  return parseAplayOutput(stdout);
};

export const promptForSoundCard = async (): Promise<SoundCardInfo> => {
  ...
  const { soundCardId } = await inquirer.prompt<{ soundCardId: string }>([
    {
      type: "list",
      name: "soundCardId",
